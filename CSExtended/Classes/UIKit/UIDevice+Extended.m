...
typedef struct {
    ...
    uint64_t awdl_in;
    uint64_t awdl_out;
} cs_net_interface_counter;


static uint64_t cs_net_counter_add(uint64_t counter, uint64_t bytes) {
...
static uint64_t cs_net_counter_get_by_type(cs_net_interface_counter *counter, CSNetworkTrafficType type) {
    uint64_t bytes = 0;
    if (type & CSNetworkTrafficTypeWWANSent) bytes += counter->pdp_ip_out;
    if (type & CSNetworkTrafficTypeWWANReceived) bytes += counter->pdp_ip_in;
    if (type & CSNetworkTrafficTypeWIFISent) bytes += counter->en_out;
    if (type & CSNetworkTrafficTypeWIFIReceived) bytes += counter->en_in;
    if (type & CSNetworkTrafficTypeAWDLSent) bytes += counter->awdl_out;
    if (type & CSNetworkTrafficTypeAWDLReceived) bytes += counter->awdl_in;
    return bytes;
}

static cs_net_interface_counter cs_get_net_interface_counter() {
    ...
    cs_net_interface_counter counter = {0};
        ...
        while (cursor) {
            if (cursor->ifa_addr->sa_family == AF_LINK) {
                ...
                if (name) {
                    ...
                    counter_in = cs_net_counter_add(counter_in, data->ifi_ibytes);
                    sharedInCounters[name] = @(counter_in);
                    
                    uint64_t counter_out = ((NSNumber *)sharedOutCounters[name]).unsignedLongLongValue;
                    counter_out = cs_net_counter_add(counter_out, data->ifi_obytes);
...
- (uint64_t)getNetworkTrafficBytes:(CSNetworkTrafficType)types {
    cs_net_interface_counter counter = cs_get_net_interface_counter();
    return cs_net_counter_get_by_type(&counter, types);
}
...
- (BOOL)passcodeStatusSupported
{
    ...
    // 当密码被删除时，秘密是否会被无效？ 如果没有，那么使用kSecAttrAccessibleWhenUnlocked
...
- (CSPasscodeStatus)passcodeStatus
{
    ...
    if (sacObject != NULL || error == NULL) {
        ...
        if (status == errSecDecode) {
            return CSPasscodeStatusDisabled;
        }
        ...
        if (status == errSecSuccess) {
            return CSPasscodeStatusEnabled;
        }
